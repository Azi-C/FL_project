...
def run_rounds(
    ...
    gamma: float = 0.20,    # reputation threshold (float, same scale as r_i^(0))
    k_aggregators: int = 1, # how many aggregators we try to pick each round
):
    if num_clients < 1:
    ...
    # ---- initial reputation r_i^(0) = |D_i| / sum |D_j| ----
    sizes = [c.num_examples() for c in clients]
    total_k = float(sum(sizes)) if sum(sizes) > 0 else 1.0  # avoid div by zero
    reputation: Dict[int, float] = {c.cid: (sizes[i] / total_k) for i, c in enumerate(clients)}

    # Try to resume from disk (if file exists)
    ...
    print(f"Init reputations r_i^(0)=|D_i|/Σ|D| -> {{cid: r for cid, r in { {c.cid: round(reputation[c.cid], 4) for c in clients} }.items()}}")
    print(f"Reputation threshold gamma = {gamma}, validation tau = {tau}")
...
if __name__ == "__main__":
    run_rounds(
        ...
        lr=0.01,
        tau=0.90,
        gamma=0.20,         # threshold compatible with r_i^(0) in [0,1]
        k_aggregators=1,
    )
